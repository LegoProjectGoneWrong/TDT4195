...
SceneNode* createSystem(){
  ...
  sun-> vertexArrayObjectID = createCircleVAO(SLICES,SLICES,1,0.6,0);
  ...
  //SceneNode* earth = createPlanet(4,0,0,0.0,0.8,0.3,1,1,y_axis,1,y_axis,sun);
  ...
  SceneNode* jupiter     = createPlanet(0,0,-10,0.7,0.8,0.7,2,0.7,y_axis,0.7,y_axis,sun);
...
void renderPlanet(SceneNode* planet){
  ...
  if(planet->main_planet){ //if sun no shade shall be added
    glUniformMatrix4fv(4,1,false,glm::value_ptr(-null_vector));
  }
  else{
