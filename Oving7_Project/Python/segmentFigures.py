...
COLOUMNS = 8
ROWS = 5
path = "images/difficult02"
img = Im.open(path+".png")
WIDTH,HEIGHT = img.size

SEGMENT_GRADE = 2 #span for segmentation
K_VALUE = 30 #segment value, if below: redo
EDGE_TRESHOLD = 40 #number of white pixels in canny edge piece for classify as object
SCAN_DISTANCE = 5 #for eroding binary image, cutting the edges
CENTRE_DISTANCE = 20 #distance to next edge parameter
ANALYZE_SAMPLES = 10 #check color of this many pixels
...
def save_parts_from_lists(parts,indexes,filepath,ending="",endings=[]):
  print("Saving")
  if endings:
    for i in range(len(parts)):
          x,y = indexes[i]
          name = filepath+"/Slot_"+str(x)+"_"+str(y)+endings[i]+".png"
...
def isolate_object(part,centres=None): #isolates a single object
  w,h = part.size
  if centres:
  ...
  attempts = 0
  while(check_corners(part_seg) and seeds and attempts <5): #max try 5 times
    attempts+=1
    part_seg = segment_color_matrix(part,Moore(),seeds.pop(),10)
  else:
    part = Im.fromarray(np.asarray(part_seg))
  part = remove_padding(part)
  part = perform_closure(np.asarray(part),Disc(5)) #smoothes the picture with closure
  part = convert_to_255(part)
...
def find_center(m):#finds the centroid
...
def get_seeds(m): #gets all white pixels
  seeds = set([])
  m = np.asarray(m)
  lx,ly = len(m),len(m[0])
  for x in range(lx):
...
def blend_images(im1,im2): #blends two binaries
...
def put_center(im,x,y,r=5): #puts white centre at cordinates
  m = np.asarray(im)
  rr,cc = draw.circle(x,y,r)
  m.setflags(write=1)
  m[rr,cc]=255
  return Im.fromarray(m)

def analyze_segmentation(part,seg): #analyze color picture from segmentation, determines COLOR
  seg = np.asarray(seg)
...
def get_ending(i):
  types = ["_star","_pacmam","_triangle","_pyramid","_square","_hex_B","_hex_W"]
  return types[i]
...
def classify_picture(path):
  ...
  save_parts_from_lists(centers,scans,path,endings=endings)
