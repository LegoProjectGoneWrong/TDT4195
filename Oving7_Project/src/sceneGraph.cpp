...
SceneNode* createSceneNode() {
	...
	node->rotationDirection = glm::vec3(0, 1, 0); //default around y axis
	node->vertexArrayObjectID = -1;
  node->main_planet = false;
  node->currentRotation = 0;
  node->selfRotationDirection = glm::vec3(0,1,0);
  node->selfRotationSpeedRadians = 0;
  node->triangleCount;
  node->number_of_figures=0;
  node->number_of_tiles=0;
  node->belongs_to_tile = 0;
  node->is_vacant = true;
	return node;
}
