...
float move_x,move_z,curr_x,curr_z,speed_x,speed_z, time_moving;
...
void createFigure(int type, SceneNode* reference_node,int x, int z ,float R, float G, float B){
  ...
  if(reference_node->children[position]->is_vacant){
    ...
    if(type==0){
      createStar(figure,0,0,1);
    }
    else if(type==1){
      createPacman(figure,1,0,0);
    }
    else if(type==2){
      createWeirdTriangle(figure,1,1,0);
    }
    else if(type==3){
      createStandardTriangle(figure,0.6,0,0.8);
    }
    else if(type==4){
      createParallelogram(figure,0,1,0.1);
    }
    else if(type==5){
      createHexagon(figure,0.2,0.2,0.2);
    }
    else if(type==6){
      createHexagon(figure,0.8,0.8,0.8);
    }
    else if(type==7){
...
void updateMovingFigurePosition(SceneNode* current_figure,glm::vec3 ref_pos, float time_elapsed){
  time_moving += time_elapsed; 
  if(time_moving<0.5){
    current_figure->y = current_figure->y -3*time_elapsed;
  }
  else if (time_moving<1.5){
    float curr_x = current_figure->x;
    float curr_z = current_figure->z;
    current_figure->x = curr_x + 5*time_elapsed*(move_x-curr_x + speed_x);
    current_figure->z = curr_z + 5*time_elapsed*(move_z-curr_z + speed_z);
  }
  else if(time_moving<2){
...
void addFiguresFromFile(SceneNode* reference_node){
  ifstream myReadFile;
  myReadFile.open("C://Users//Miklel//Documents//Work//4.Visuell//Oving7_Project//Python//objects.txt");
  string STRING;
  if (myReadFile.is_open()) {
    printf("Reading for file\n");
    while (!myReadFile.eof()) {
      ...
      createFigure(std::stoi(arr[0]),reference_node,std::stoi(arr[1]),std::stoi(arr[2]),random(),random(),random());
    }
  }
  myReadFile.close();
}
...
void runProgram(GLFWwindow* window){
    ...
    addFiguresFromFile(reference_node);
...
void setWindowSettings(GLFWwindow* window){
    ...
    Gloom::Shader shader;
    shader.makeBasicShader("C://Users//Miklel//Documents//Work//4.Visuell//Oving7_Project//Project//gloom//gloom//shaders//simple.frag","C://Users//Miklel//Documents//Work//4.Visuell//Oving7_Project//Project//gloom//gloom//shaders//simple.vert");
    shader.activate();
